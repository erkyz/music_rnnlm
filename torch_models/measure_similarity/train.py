...
def train():
    ...
    for batch in train_mb_indices:
        if args.arch == 'crnn':
            data, conditions, targets = get_batch_variables(train_batches, train_targets, train_measure_weights, train_measure_steps, batch)
        else:
        ...
        if args.arch == 'rrnn':
            ...
            outputs, hidden = model(data, conditions, hidden)
        else:
            outputs, hidden = model(data, hidden)
