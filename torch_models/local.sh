...
# TMP_PREFIX="010_001"
#DATA_DIR="../music_data/CMaj_Nottingham/" 
#TMP_PREFIX="CMaj_Nottingham_cnn"
VOCAB_PATHS=\[\"$DATA_DIR\"\]

LR=0.001
python train.py --nhid=$NHID --vocab_paths=$VOCAB_PATHS --path=$DATA_DIR --tmp_prefix=$TMP_PREFIX --batch_size=$BSZ --c=$C --distance_threshold=$DISTANCE_THRESHOLD --arch=$ARCH --rnn_type=$RNN_TYPE --seed=$SEED --lr=$LR --emsize=$EMSIZE --dropout=$DROP --skip_first_n_note_losses=$SKIP --nlayers=$NL --epochs=$EPOCHS --metaf='meta.p' --train_info_out=$2 --temperature=0.5 --save=$SAVE --baseline --synth_data # --cnn_encoder --synth_data
