...
# GENERATE
TEMPERATURE=1.0
python main.py --mode="generate" --nhid=$NHID --vocab_paths=$VOCAB_PATHS --path=$DATA_DIR --batch_size=$BSZ --arch=$ARCH --rnn_type=$RNN_TYPE --seed=$SEED --lr=$LR --emsize=$EMSIZE --dropout=$DROP --skip_first_n_note_losses=$SKIP --nlayers=$NL --epochs=$EPOCHS --metaf='meta.p' --input_feed_num=$INPUT_FEED --temperature=$TEMPERATURE
